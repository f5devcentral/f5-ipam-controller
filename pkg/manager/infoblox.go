...
const EAKey = "F5-IPAM"
const EAVal = "managed"
const httpRequestTimeout = 20
const httpPoolConnections = 10
...
// InfobloxParams allows NewInfobloxManager to receive all or some parameters
...
// NewInfobloxManager sets up an interface with Infoblox
func NewInfobloxManager(
	...
	transportConfig := ibclient.NewTransportConfig(params.SSLVerify, httpRequestTimeout, httpPoolConnections)
...
func (iClient *IBloxClient) CreateARecord(params ...interface{}) bool {
	name := params[0].(string)
	ipAddr := params[1].(string)
	netview := params[2].(string)
...
func (iClient *IBloxClient) DeleteARecord(params ...interface{}) {
	name := params[0].(string)
	ipAddr := params[1].(string)
	netview := params[2].(string)
	cidr := params[3].(string)
	obj := ibclient.NewRecordA(
		ibclient.RecordA{
			Name:     name,
			Ipv4Addr: ipAddr,
...
func (iClient *IBloxClient) GetNextAddr(netview, cidr string) string {
	nextAddr, err := iClient.objMgr.AllocateIP(netview, cidr, "", "", "", iClient.ea)
	if err != nil {
		return ""
	}
...
// AllocateIPAddress an IP address back to Infoblox
func (iClient *IBloxClient) AllocateIPAddress(cidr, ipAddr string) bool {
	return true
}

// GetIPAddress an IP address back to Infoblox
func (iClient *IBloxClient) GetIPAddress(hostname string) string {
	return "PASS"
}

// GetNextIPAddress an IP address back to Infoblox
func (iClient *IBloxClient) GetNextIPAddress(cidr string) string {
	return "PASS"
}

// ReleaseIPAddress an IP address back to Infoblox
func (iClient *IBloxClient) ReleaseIPAddress(ipAddr string) {
	return
}
